...
def main():
    ...
    commands = {
        ...
        "export": {
            ...
            "cmd": ["pytest", f"{benchmarks_dir}/test_benchmarks.py::TestExportBenchmarks", "--benchmark-only", "-v"]
        },
        "scaling": {
            "description": "Run scalability benchmarks",
            "cmd": ["pytest", f"{benchmarks_dir}/test_benchmarks.py::TestScalabilityBenchmarks", "--benchmark-only", "-v"]
        },
        "memory": {
            "description": "Run memory benchmarks",
            "cmd": ["pytest", f"{benchmarks_dir}/test_benchmarks.py::TestMemoryBenchmarks", "--benchmark-only", "-v"]
        },
        "build": {
            "description": "Run build benchmarks (requires python-build, cmake, compiler)",
            "cmd": ["pytest", f"{benchmarks_dir}/test_benchmarks.py::TestBuildBenchmarks", "--benchmark-only", "-v"]
        },
        "compare": {
            "description": "Run benchmarks and save for comparison",
            "cmd": ["pytest", str(benchmarks_dir), "--benchmark-only", "-v", "--benchmark-autosave", "--benchmark-save=baseline"]
        },
        "histogram": {
            "description": "Generate benchmark histogram",
            "cmd": ["pytest", str(benchmarks_dir), "--benchmark-only", "--benchmark-histogram=benchmark_histogram"]
        },
    }
